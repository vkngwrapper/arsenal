...
func (m *tlsfBlockMetadata) AddDetailedStatistics(stats *memory.DetailedStatistics) {
	stats.BlockCount++
...
func (m *tlsfBlockMetadata) AddStatistics(stats *memory.Statistics) {
	stats.BlockCount++
...
func (m *tlsfBlockMetadata) checkBlock(
	...
	alignedOffset := utils.AlignUp(block.offset, allocAlignment)
...
func (m *tlsfBlockMetadata) PrintDetailedMapHeader(json jwriter.ObjectState) error {
	...
	var stats memory.DetailedStatistics
	stats.Clear()
...
func (m *tlsfBlockMetadata) CheckCorruption(blockData unsafe.Pointer) (common.VkResult, error) {
	for block := m.nullBlock.prevPhysical; block != nil; block = block.prevPhysical {
		if !block.IsFree() {
			if !utils.ValidateMagicValue(blockData, block.offset+block.size) {
