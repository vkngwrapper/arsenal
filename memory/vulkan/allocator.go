...
type VulkanAllocator struct {
	...
	memoryCallbacks     *memory.MemoryCallbackOptions
...
func (a *VulkanAllocator) calcAllocationParams(
	o *memory.AllocationCreateInfo,
	requiresDedicatedAllocation bool,
	prefersDedicatedAllocation bool,
) (common.VkResult, error) {
	hostAccessFlags := o.Flags & (memory.AllocationCreateHostAccessSequentialWrite | memory.AllocationCreateHostAccessRandom)
	if hostAccessFlags == (memory.AllocationCreateHostAccessSequentialWrite | memory.AllocationCreateHostAccessRandom) {
	...
	if hostAccessFlags == 0 && (o.Flags&memory.AllocationCreateHostAccessAllowTransferInstead) != 0 {
	...
	if o.Usage == memory.MemoryUsageAuto || o.Usage == memory.MemoryUsageAutoPreferDevice || o.Usage == memory.MemoryUsageAutoPreferHost {
		if hostAccessFlags == 0 && o.Flags&memory.AllocationCreateMapped != 0 {
	...
	if requiresDedicatedAllocation || o.Usage == memory.MemoryUsageGPULazilyAllocated {
		o.Flags |= memory.AllocationCreateDedicatedMemory
	}
	...
	if o.Flags&memory.AllocationCreateDedicatedMemory != 0 && o.Flags&memory.AllocationCreateNeverAllocate != 0 {
	...
	if o.Usage != memory.MemoryUsageAuto && o.Usage != memory.MemoryUsageAutoPreferDevice && o.Usage != memory.MemoryUsageAutoPreferHost {
		if hostAccessFlags == 0 {
			o.Flags |= memory.AllocationCreateHostAccessRandom
		}
	}
...
func (a *VulkanAllocator) findMemoryPreferences(
	...
	switch o.Usage {
	case memory.MemoryUsageGPULazilyAllocated:
		requiredFlags |= core1_0.MemoryPropertyLazilyAllocated
		break
	case memory.MemoryUsageAuto, memory.MemoryUsageAutoPreferDevice, memory.MemoryUsageAutoPreferHost:
		{
			...
			hostAccessSequentialWrite := o.Flags&memory.AllocationCreateHostAccessSequentialWrite != 0
			hostAccessRandom := o.Flags&memory.AllocationCreateHostAccessRandom != 0
			hostAccessAllowTransferInstead := o.Flags&memory.AllocationCreateHostAccessAllowTransferInstead != 0
			preferDevice := o.Usage == memory.MemoryUsageAutoPreferDevice
			preferHost := o.Usage == memory.MemoryUsageAutoPreferHost
...
func (a *VulkanAllocator) calculateMemoryTypeParameters(
	options *memory.AllocationCreateInfo,
	memoryTypeIndex int,
	size int,
	allocationCount int,
) (common.VkResult, error) {
	// If memory type is not HOST_VISIBLE, disable MAPPED
	if options.Flags&memory.AllocationCreateMapped != 0 &&
		a.deviceMemory.MemoryTypeProperties(memoryTypeIndex).PropertyFlags&core1_0.MemoryPropertyHostVisible == 0 {
		options.Flags &= ^memory.AllocationCreateMapped
	}

	// Check budget if appropriate
	if options.Flags&memory.AllocationCreateDedicatedMemory != 0 &&
		options.Flags&memory.AllocationCreateWithinBudget != 0 {
...
func (a *VulkanAllocator) allocateMemoryOfType(
	...
	dedicatedBufferUsage core1_0.BufferUsageFlags,
	dedicatedImage core1_0.Image,
	createInfo memory.AllocationCreateInfo,
	memoryTypeIndex int,
	suballocationType metadata.SuballocationType,
	dedicatedAllocations *dedicatedAllocationList,
	...
	mappingAllowed := finalCreateInfo.Flags&(memory.AllocationCreateHostAccessSequentialWrite|memory.AllocationCreateHostAccessRandom) != 0

	if finalCreateInfo.Flags&memory.AllocationCreateDedicatedMemory != 0 {
		return a.allocateDedicatedMemory(
			...
			memoryTypeIndex,
			finalCreateInfo.Flags&memory.AllocationCreateMapped != 0,
			mappingAllowed,
			finalCreateInfo.Flags&memory.AllocationCreateCanAlias != 0,
	...
	canAllocateDedicated := finalCreateInfo.Flags&memory.AllocationCreateNeverAllocate == 0 && (pool == nil || !blockAllocations.HasExplicitBlockSize())
...
func (a *VulkanAllocator) AllocateMemory(memoryRequirements core1_0.MemoryRequirements, o memory.AllocationCreateInfo, outAlloc *Allocation) (common.VkResult, error) {
